 ...
 */
var queue        = [];
//i = the number in the array queue[]
var i = 0;
//I create var first to test the first path in the algorithms
var first = true;
//urlNew to add the new url in a text file
var urlNew;
/**
...
function extract_links(page_url, html_str){
  ...
  // So here I do "(match() || []) in order to always work on an array (and yes, that's another pattern).s
  ...
  (html_str.match(EXTRACT_URL_REG) || []).forEach(function(url){
    ...
    //I check if I have already crawled this url
    if (searchQueue(url) == true && first == false) {
      ;
    }
    else{
      //Here I add the url in the queue and i write the url in the file text 
      first = false;
      queue.push(url);
      writeFile(url);
      //And I continue to crawle the page
      em.emit('url', page_url, html_str, url);  
    } 
  });
  //And I continue in a new page
  if(get_page(queue[i]) == EXTRACT_URL_REG) {
    get_page(queue[i])
    i++;
  }
  else{
...
//Check if the the queue know the url
function searchQueue(url){
...
//Function to retrieve the data header but i couldn't use
function extractHeader(url){
  var xhr_object = null; 
...
//I add the url in the url text file
function writeFile(url){
  var fs = require('fs');
  urlNew += "\n"+url;
  fs.writeFile("url.txt", urlNew, function(err) {
    if(err){
        console.log(err);
    } else {
...
//Sometimes I have error when the scraper scan url but I could not finish
...
get_page('https://www.lavache.com');

/*
1/ Fais en sorte que ton scraper envoie une page HTML, contenant un formulaire HTML lorsque l'on va sur la page "/home"
2/ Le formulaire HTML doit faire un method="POST" sur /home
3/ Dans ton express, fait app.post('/home', function(){
...
// Ici ton code va pouvoir récupérer les paramètres envoyés en POST à ton "/home" depuis ton formulaire :)
// Check google, sur comment récupérer les paramètres POST avec "express"
...
});*/
